...
class PipelineSession:
    ...
    async def _run_llm(self, user_text: str) -> str:
        ...
        # Add greeting as assistant message if needed
        # messages.append({"role": "assistant", "content": cfg["greeting"]})
...
async def handle_client(ws):
    ...
    logger.info(f"[Multi] New connection from {client_ip}")

    session = PipelineSession(ws)
    
    # Restore history if exists
    if client_ip in GLOBAL_SESSIONS:
        logger.info(f"[Multi] Restoring session for {client_ip} ({len(GLOBAL_SESSIONS[client_ip])} msgs)")
    ...
    try:
        ...
        logger.info(f"[Multi] Saved session for {client_ip}")
